...
def gui():
    ...
    root.iconbitmap(
        'C:/Users/Dell/Desktop/GIT/TRACKORONA---DESKTOP-APP---master/covid_icon_2_FMC_icon.ico')
    root.configure(background='red')
    root.geometry("400x500")

    def graph():
        # Web scrapping to get the graph data set
        html_graph = requests.get(
    ...
    def vaccine():
        ...
        result = str(soup.find_all(
            "div", class_="is_h5-2 is_developer w-richtext"))
        root2 = Tk()
        root2.title("VACCINE IN PROGRESS")
        # root2.geometry("600x190")
        # T = Text(root2, height=100, width=100,
        #          font='Times', bg="Yellow", fg="Black")
        # take the data
    #     lst = [(1, result[46:86].replace(" ", "")),
    #    (2, result[139:199].replace(" ", "")),
    #    (3, result[279:305].replace(" ", "")),
    #    (4, result[358:374].replace(" ", "")),
    #    (5, result[428:436].replace(" ", ""))]
        lst=[]
        result2=soup.find_all(
            "div", class_="is_h5-2 is_developer w-richtext")
        result3=soup.find_all(
            "div", class_="is_h5-2 is_treatments w-richtext")
        lst=[("SNO","VACCINE DESCRIPTION","VACCINE PRODUCER")]
        for i in range(32):
           list=[]
            
           b=result2[i].decode_contents()
           a= result3[i].decode_contents()
           list.append(i+1)
           list.append(a)
           list.append(b)
           t=tuple(list)
...
# columns in list
        ...
        for i in range(total_rows):
            for j in range(total_columns):
                 if i==0 and j==0:
                   e = Entry(root2, width=4, fg='blue',
                               font=('Arial',13,'bold'),justify=CENTER)
                   e.grid(row=i, column=j)
                   e.insert(END, lst[i][j])
                   continue
                 if i==0 and j==1: 
                   e = Entry(root2, width=85, fg='blue',
                               font=('Arial',10,'bold'),justify=CENTER)
                   e.grid(row=i, column=j)
                   e.insert(END, lst[i][j])
                   continue 
                 if i==0 and j==2: 
                     e = Entry(root2, width=128, fg='blue',
                               font=('Arial',10,'bold'),justify=CENTER)
                     e.grid(row=i, column=j)
                     e.insert(END, lst[i][j])
                     continue      
